...
def test_unauth_user_get_all(client, test_posts):
    res = client.get("/posts/")
    assert res.status_code == 401


def test_unauth_user_get_one(client, test_posts):
    res = client.get(f"/posts/{test_posts[0].id}")
    assert res.status_code == 401


def test_get_one_post_not_exists(authorized_client, test_posts):
    res = authorized_client.get(f"/posts/7777")
...
@pytest.mark.parametrize("title, content, published", [
    ...
    ("Best Content", "Come On Pizza", True),
])
...
def test_unauth_user_create_post(client, test_user, test_posts):
    res = client.get("/posts/", json={"title": "Any Title", "content": "Any Content"})
    assert res.status_code == 401
...
def test_delete_post_nonexist(authorized_client, test_user, test_posts):
    res = authorized_client.delete(f"/posts/888888888888888888")
...
def test_update_post_nonexist(authorized_client, test_user, test_posts):
    ...
    res = authorized_client.put(f"/posts/888888888888888888", json=data)
